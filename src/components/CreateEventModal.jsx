...
export default function CreateEventModal() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!isAdmin) {
      showToast(`🔒 Event posting is restricted to Admin (${ADMIN_EMAIL})`, 'info');
        ...
        <div className="p-6 sm:p-8 space-y-6 max-h-[85vh] overflow-y-auto">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Publish comprehensive event posts with custom posters, dates, venue settings, track rules, and team limits ({ADMIN_EMAIL}).
            </p>
          </div>
          ...
          {!isAdmin ? (
            <div className="p-6 rounded-xl bg-amber-950/40 border border-amber-800/40 text-amber-200 text-xs space-y-3">
              ...
              <p>
                Events can only be published by the official TechZen admin: <strong className="text-white font-mono">{ADMIN_EMAIL}</strong>.
              </p>
            </div>
          ) : (
