        ...
        <div className="p-8 space-y-6">
          ...
          <div className="text-center space-y-3">
            ...
            <h2 className="text-xl font-semibold text-white font-sans">
              {currentMode === 'login' ? 'Welcome back, builder' : 'Join the signal'}
            </h2>
            <p className="text-xs text-white/55 font-sans">
              {currentMode === 'login' ? 'Sign in with your email or Google account.' : 'Create your TechZen builder account.'}
            </p>
          </div>
          ...
          <button
            ...
            className="w-full py-3 px-4 rounded-md border border-white/15 bg-white/[.04] hover:bg-white/[.08] text-white text-xs font-semibold transition flex items-center justify-center space-x-2.5 cursor-pointer hover:border-[#ef2635]/50"
          >
          ...
          <div className="relative flex items-center justify-center">
            ...
            <span className="absolute bg-[#111111] px-3 text-[11px] font-mono text-white/40 uppercase">or</span>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {currentMode === 'signup' && (
              <div>
                <label className="block text-xs text-white/65 mb-1">Full Name</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3.5 py-2.5 rounded-md border border-white/15 bg-white/[.04] text-white text-xs focus:outline-none focus:border-[#ef2635]"
                />
              </div>
            )}

            <div>
              <label className="block text-xs text-white/65 mb-1">Email address</label>
              <input
                type="email"
                required
            ...
            <div>
              <label className="block text-xs text-white/65 mb-1">Password</label>
              <input
                type="password"
                required
                placeholder="••••••••"
            ...
            {currentMode === 'signup' && (
              <div>
                <label className="block text-xs text-white/65 mb-1">Community Role</label>
                <select
                  value={role}
                  onChange={(e) => setRole(e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-md border border-white/15 bg-[#191919] text-white text-xs focus:outline-none focus:border-[#ef2635]"
                >
                  <option value="Attendee">Attendee / Member</option>
                  <option value="Organizer">Event Host / Organizer</option>
            ...
            <button
              type="submit"
              className="w-full py-3 px-4 rounded-md bg-[#ef2635] hover:bg-[#ff3d4b] text-white font-semibold text-xs tracking-wider transition flex items-center justify-center space-x-1.5 cursor-pointer"
            >
              <span>{currentMode === 'login' ? 'Continue ►' : 'Create Account ►'}</span>
            </button>
          </form>

          {/* Toggle link */}
          <div className="text-center text-xs text-white/45 pt-2 border-t border-white/10">
            {currentMode === 'login' ? (
              <span>
                ...
                <button
                  ...
                  onClick={() => {
                    setAuthMode('signup');
                    if (isSignInPath) setLocation('/sign-up');
                  }}
                  className="text-[#ff4c59] font-semibold hover:underline cursor-pointer"
                >
                  Sign up
                </button>
              </span>
              ...
              <span>
                ...
                <button
                  ...
                  onClick={() => {
                    setAuthMode('login');
                    if (isSignUpPath) setLocation('/sign-in');
                  }}
                  className="text-[#ff4c59] font-semibold hover:underline cursor-pointer"
                >
                  Sign in
